...
import { EmbedBuilder, SlashCommandBuilder, SlashCommandMentionableOption, SlashCommandStringOption, SlashCommandUserOption} from 'discord.js';
...
export async function execute(interaction) {
    ...
    await interaction.guild.channels.fetch().then(async listChannels => {
      ...
      let date
      let dateFormat
      for ( let abscisseTimeStamp = interaction.guild.createdTimestamp; abscisseTimeStamp < interaction.createdTimestamp; abscisseTimeStamp += constanteTemps) {
        dateFormat = new Date(abscisseTimeStamp)
        date = dateFormat.getDate()+ "/"+ (dateFormat.getMonth() + 1)+ "/"+ dateFormat.getFullYear()
        abscisse.push(date)
        ordonnee.push(userMessages.filter( element => element.timestamp < abscisseTimeStamp + constanteTemps && element.timestamp > abscisseTimeStamp ).length)
        let debug = { test : abscisseTimeStamp, temoin : interaction.createdTimestamp }
        //console.log(debug)
      }
      ...
      for ( let i = 0; i<listNomChannels.length; i++ ) {
        ...
        let channelMessage = userMessages.filter( message => message.channelId == nomChannelId)
        let elemt = {
      ...
      for ( let i = 0; i < 3; i++ ) {
        contentMessage = `${contentMessage}**${(i + 1)}.** ${messageParChannels[i].name} : ${messageParChannels[i].count}\n`
      }
  ...
  // ============================== Graphe =======================================
      ...
      let channel = interaction.client.channels.cache.find(channel => channel.id == channelIdToSend)
      channel.send({
        embeds : [Embed]
      })
